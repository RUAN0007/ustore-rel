/* 
   ...
   @Last_Modified_At:   2017-01-23 17:43:04
...
class TupleDscp {
 public:
    ...
    TupleDscp(std::string schema_name,
              std::vector<const Type*> types,
              std::vector<std::string> field_names);

    TupleDscp(std::string schema_name, unsigned pk_index,
              std::vector<const Type*> types,
              std::vector<std::string>  field_names);
...
class Tuple {
 public:
    ...
    Tuple(unsigned char* back_store,
          unsigned start_position, const TupleDscp* schema);
    ...
    // msg will contain the message of this operation,
    // e.g, failure reason or succeeded.
    bool SetFieldByName(std::string field_name,
                        Field* new_field, std::string* msg);
    ...
    inline const unsigned char* GetRawBytes(const size_t& tuple_size) const {
    ...
    inline bool IsSatisfy(const Predicate* predicate) const  {
        ...
        bool isTrue = field->IsSatisified(
                                        predicate->GetOp(),
                                        predicate->GetField());
        delete field;
    ...
    inline friend bool operator== (const Tuple &lhs, const Tuple &rhs) {
        ...
        return memcmp(lhs.GetRawBytes(tuple_size),
                      rhs.GetRawBytes(tuple_size),
                      tuple_size) == 0;
    }
...
class PageIterator: public Tuple::Iterator {
 public:
    PageIterator(const std::string& relation_name,
                 ClientService* client, Page* stored_page,
 ...
 private:
    // Given a vector of RecordID, group the RecordID
    // with same ustore version together
    // Set the tuple_pos
