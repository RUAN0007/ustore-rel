...
TEST_F(ClientTest, SyncGetSinglePreVersion) {
	...
	pair<version_t,version_t> pre_versions = client.SyncGetPreviousVersion("key", v2);
...
TEST_F(ClientTest, SyncGetSinglePreValue) {
	...
	pair<value_t*, value_t*> pre_values = client.SyncGetPreviousValue("key", v2);
...
TEST_F(ClientTest, SyncGetBothPreValue) {
	...
	pair<value_t*, value_t*> pre_values = client.SyncGetPreviousValue("key", v4);
...
TEST_F(ClientTest, SyncGetNonePreValue) {
	...
	version_t v5 = 5;
	pair<value_t*, value_t*> pre_values = client.SyncGetPreviousValue("key", v5);
