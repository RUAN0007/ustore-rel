/* 
   ...
   @Last_Modified_At:   2017-01-09 10:43:31
...
value_t* ClientService::SyncGet(const key_t& key, const version_t& version){
	...
	if(objects_[key].values_.count(version) == 0) return new value_t(NULL_VALUE);

	return new value_t(objects_[key].values_[version]);
}

version_t ClientService::SyncMerge(const key_t& key, const version_t& pre_version_1, const version_t& pre_version_2, 
                const value_t& value){
	...
	objects_[key].parents_[new_version] = make_pair(pre_version_1, pre_version_2);
...
std::pair<version_t, version_t> ClientService::SyncGetPreviousVersion(
  const key_t& key, const version_t& version){
	...
	if(objects_[key].values_.count(version) == 0) return make_pair(NULL_VERSION, NULL_VERSION);
...
std::pair<value_t*, value_t*> ClientService::SyncGetPreviousValue(
  const key_t& key, const version_t& version){
	...
	pair<version_t, version_t> pre_versions = SyncGetPreviousVersion(key, version);
