/*
   ...
   @Last_Modified_At:   2017-01-24 13:06:25
...
TupleDscp::TupleDscp(std::string schema_name,
...
bool operator== (const TupleDscp &lhs, const TupleDscp &rhs) {
    ...
    while (lhs_field_item_it != lhs.inner_schema_.end() ||
           rhs_field_item_it != rhs.inner_schema_.end()) {
...
Tuple::Tuple(unsigned char* back_store,
...
bool Tuple::SetFieldByName(string field_name,
                           Field* new_field,
                           std::string* msg) {
...
PageIterator::PageIterator(const string& relation_name,
